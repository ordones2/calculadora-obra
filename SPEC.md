...
> Implementação de referência: `src/lib/calc/engine.ts` (função pura `calcular`), testada
> em `src/lib/calc/engine.test.ts`.
...
- **Proposta em PDF** com a marca do arquiteto (`src/pdf/Proposta.tsx`).
- **Contato/lead**: link de WhatsApp (sem backend nesta fase).

## 6. Arquitetura

- **Stack**: React + Vite + TypeScript + Tailwind. App 100% estático (deploy em
  Vercel/Netlify/GitHub Pages).
- **Sem banco de dados**:
  - Seed oficial em `src/data/config.default.json` (versionado no git) — é a tabela publicada.
  - Edições do arquiteto persistem em `localStorage` (`src/lib/config/store.ts`).
  - **Exportar/Importar JSON** + **Restaurar padrão** = fluxo de "publish" (commitar o seed).
- **Roteamento**: `react-router` em modo hash (`/`, `/resultado`, `/admin`).
- **Estado**: `ConfigProvider` (config ativa) + `sessionStorage` para a entrada do wizard.

### Modelo de dados (`Config`)

Tipos em `src/types/config.ts`. Estrutura: `meta`, `marca`, `padroes[]`, `fatores`
...
Entrada/saída do cálculo em `src/types/calc.ts` (`CalcInput`, `CalcResult`).
...
- **Fase 0 — Esqueleto (entregue)**: scaffold, tipos, seed, motor de cálculo + testes, store
  de config (seed + localStorage + export/import), wizard funcional (estimativa rápida + por
  ambiente), resultado, PDF inicial, admin (marca/padrões/BDI + editor JSON).
...
1. Wizard calcula e leva ao `/resultado` com faixa, orçamento e cronograma.
...
4. Baixar a proposta em PDF.
5. `npm test` e `npm run build` concluem sem erros.
